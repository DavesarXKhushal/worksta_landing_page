...
const HomePage = () => {
    ...
    return (
        <section className="w-full relative flex items-center justify-center flex-col px-4 md:px-0 py-8">
                            ...
                            <nav className="hidden md:flex relative items-center justify-center mt-8 md:mt-12 w-full">
                                <Link href="#" className="flex items-center justify-center w-max rounded-full border-t border-foreground/30 bg-white/20 backdrop-blur-lg px-2 py-1 md:py-2 gap-2 md:gap-8 shadow-3xl shadow-background/40 cursor-pointer select-none">
                                    <p className="text-foreground text-sm text-center md:text-base font-medium pl-4 pr-4 lg:pr-0">
                                        ✨  Start today: browse nearby shifts or post your first job
                                    </p>
            ...
            <Wrapper className="flex flex-col items-center justify-center py-12 relative">
                <Container>
                    <header className="max-w-md mx-auto text-start md:text-center">
                        ...
                        <h2 className="text-3xl lg:text-4xl font-semibold mt-6">Three steps to get staffed</h2>
                        <p className="text-muted-foreground mt-6">Create a profile, book or post shifts, and get paid weekly.</p>
                    </header>
                </Container>
                <Container>
            ...
            <Wrapper className="flex flex-col items-center justify-center py-12 relative">
                ...
                <Container>
                    <header className="max-w-md mx-auto text-start md:text-center">
                        ...
                        <h2 className="text-3xl lg:text-4xl font-semibold mt-6">Discover our powerful features</h2>
                        <p className="text-muted-foreground mt-6">Astra connects F&B venues and Pros with fast, reliable staffing.</p>
                    </header>
                </Container>
                <Container>
            ...
            {/* pricing */}
            <Wrapper className="flex flex-col items-center justify-center py-12 relative">
                <span className="hidden md:block absolute top-0 -right-1/3 w-72 h-72 bg-blue-500 rounded-full blur-[10rem] -z-10"></span>
                <Container>
                    <header className="max-w-md mx-auto text-start md:text-center">
                        <SectionBadge title="Pricing" />
                        <h2 className="text-3xl lg:text-4xl font-semibold mt-6">Unlock the right plan for your business</h2>
                        <p className="text-muted-foreground mt-6">Choose the best plan for your venue and start hiring today.</p>
                    </header>
                </Container>
                <Container className="flex items-center justify-center">
                    <section className="grid grid-cols-1 lg:grid-cols-3 gap-5 w-full md:gap-8 py-10 md:py-20 flex-wrap max-w-4xl">
                        {pricingCards.map((card) => (
                            <Card key={card.title} className={cn("flex flex-col w-full border-neutral-700", card.title === "Business" && "border-2 border-primary")}>
                                <CardHeader className="border-b border-border">
                                    <span>{card.title}</span>
                                    <CardTitle className={cn(card.title !== "Business" && "text-muted-foreground")}>{card.price}</CardTitle>
                                    <CardDescription>{card.description}</CardDescription>
                                </CardHeader>
                                <CardContent className="pt-6 space-y-3">
                                    {card.features.map((feature) => (
                                        <article key={feature} className="flex items-center gap-2">
                                ...
                                <CardFooter className="mt-auto">
                                    <Link href="#" className={cn("w-full text-center text-primary-foreground bg-primary p-2 rounded-md text-sm font-medium", card.title !== "Business" && "!bg-foreground !text-background")}>
                                        {card.buttonText}
                                    </Link>
                                </CardFooter>
                            </Card>
            ...
            <Wrapper className="flex flex-col items-center justify-center py-12 relative">
                ...
                <Container>
                    <header className="max-w-md mx-auto text-start md:text-center">
                        ...
                        <p className="text-muted-foreground mt-6">See how Astra empowers venues and Pros of all sizes.</p>
                    </header>
                </Container>
                <Container>
                    <section className="py-10 md:py-20 w-full">
                        <article className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden py-10">
                            <Marquee pauseOnHover className="[--duration:20s] select-none">
                                {firstRow.map((review) => (
                                    <figure key={review.name} className={cn("relative w-64 cursor-pointer overflow-hidden rounded-xl border p-4", "border-zinc-50/[.1] bg-background over:bg-zinc-50/[.15]")}> 
                ...
                <Container>
                    <LampContainer>
                        <section className="flex flex-col items-center justify-center relative w-full text-center">
                            <h2 className="text-4xl lg:text-5xl xl:text-6xl lg:!leading-snug font-semibold mt-8">From Idea to Launch <br /> Faster Than Ever</h2>
                            <p className="text-muted-foreground mt-6 max-w-md mx-auto">Build stunning experiences with Astra&apos;s intuitive tools.</p>
                    ...
                    <section className="flex items-center justify-center w-full -mt-40">
                        <article className="flex flex-col md:flex-row items-start md:items-center justify-start md:justify-between w-full px-4 md:px-8 rounded-lg lg:rounded-2xl border border-border/80 py-4 md:py-8">
                            <header className="flex flex-col items-start gap-4 w-full">
                                ...
                                <p className="text-base text-muted-foreground">Be up to date with everything about staffing</p>
                            </header>
                            <aside className="flex flex-col items-start gap-2 md:min-w-80 mt-5 md:mt-0 w-full md:w-max">
                                <form action="#" className="flex flex-col md:flex-row items-center gap-2 w-full md:max-w-xs">
                                    <Input required type="email" placeholder="Enter your email" className="focus-visible:ring-0 focus-visible:ring-transparent focus-visible:border-primary duration-300 w-full" />
                                    <Button type="submit" size="sm" variant="secondary" className="w-full md:w-max">Subscribe</Button>
                                </form>
                                <p className="text-xs text-muted-foreground">By subscribing you agree with our <Link href="#">Privacy Policy</Link></p>
                            </aside>
                        </article>
                    </section>
                </Container>
